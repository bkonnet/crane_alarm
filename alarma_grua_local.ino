...
#define ALARM_AUTO_SILENCE_MS 30000UL
#define ALARM_REARM_DELAY_MS 1000UL

bool alarmaActiva = false;
bool alarmaLatch  = false;

uint32_t alarmaStartMs = 0;
uint32_t alarmRearmBlockUntilMs = 0;
...
void procesarDato(const String& tramaIn) {
  ...
  if (id == "55") {
    pata55 = (int)distancia;
    miss55 = 0;
  } else if (id == "56") {
    pata56 = (int)distancia;
    miss56 = 0;
  } else if (id == "57") {
    pata57 = (int)distancia;
    miss57 = 0;
  } else if (id == "07") {
    pata07 = (int)distancia;
    miss07 = 0;
  }
...
void evaluarAlarma() {
  ...
  if (!alarmaActiva && algunaBaja && now >= alarmRearmBlockUntilMs) {
    alarmaActiva = true;
    alarmaLatch = true;
    alarmaStartMs = now;
  ...
  if (alarmaActiva && (now - alarmaStartMs >= ALARM_AUTO_SILENCE_MS)) {
    alarmaActiva = false;
    alarmaLatch = false;
    alarmMaskLatched = 0;
    alarmRearmBlockUntilMs = now + ALARM_REARM_DELAY_MS;

    uiMsgLine1 = "AUTO";
    uiMsgLine2 = "SILENCIO";
    uiMsgUntilMs = now + 1500;

    Serial.println(">>> AUTO SILENCIO 30s");
  }

  if (alarmaActiva && todasAltas) {
    alarmaActiva = false;
    Serial.println(">>> CONDICION SEGURA");
  }
...
void printOffSensors() {
  ...
  if (miss55 >= MISS_LIMIT) {
    display.print("01 ");
    any = true;
  }
  if (miss56 >= MISS_LIMIT) {
    display.print("02 ");
    any = true;
  }
  if (miss57 >= MISS_LIMIT) {
    display.print("03 ");
    any = true;
  }
  if (miss07 >= MISS_LIMIT) {
    display.print("04 ");
    any = true;
  }
...
void procesarComandoRadio(const String& cmd) {
  if (cmd == "BORRA") {
    ...
    alarmaLatch = false;
    alarmMaskLatched = 0;
    alarmRearmBlockUntilMs = millis() + ALARM_REARM_DELAY_MS;
